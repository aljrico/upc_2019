...
```{r}
pvalue <- wilcox.test( x = clean_data[clean_data$protein == "Normal", ]      %>% .$Weight_1,
								y = clean_data[clean_data$protein == "Proteinuria", ] %>% .$Weight_1)$p.value
...
```{r}
pvalue <- wilcox.test( x = clean_data[clean_data$protein == "Normal", ]      %>% .$Age_at_diagnosis_CKD,
								y = clean_data[clean_data$protein == "Proteinuria", ] %>% .$Age_at_diagnosis_CKD)$p.value
...
pvalue <- wilcox.test( x = clean_data[clean_data$Sex == "M", ]      %>% .$UPCR1,
								y = clean_data[clean_data$Sex == "F", ] %>% .$UPCR1)$p.value
...
```{r}
pvalue <- wilcox.test( x = clean_data[clean_data$CKD == "Y", ]      %>% .$UPCR1,
								y = clean_data[clean_data$CKD == "N", ] %>% .$UPCR1)$p.value
...
```{r}
pvalue <- wilcox.test( x = clean_data[clean_data$protein == "Normal", ]      %>% .$Crea_BQ1,
								y = clean_data[clean_data$protein == "Proteinuria", ] %>% .$Crea_BQ1)$p.value
...
```{r}
pvalue <- wilcox.test( x = clean_data[clean_data$protein == "Normal", ]      %>% .$Urea1,
								y = clean_data[clean_data$protein == "Proteinuria", ] %>% .$Urea1)$p.value
...
wilcox.test(x = clinical_signs[clinical_signs$variable != "CONTROL" & clinical_signs$value == TRUE,] %>% .$UPCR1,
							 y = clinical_signs[clinical_signs$variable == "CONTROL"  & clinical_signs$value == TRUE,] %>%  .$UPCR1)$p.value

variables <- clinical_signs$variable %>% unique() %>% as.vector()
pvalues <- c()
for(i in seq_along(variables)){
	pvalues[[i]] <- wilcox.test(x = clinical_signs[clinical_signs$variable == variables[[i]] & clinical_signs$value == TRUE,] %>% .$UPCR1,
																 y = clinical_signs[clinical_signs$variable == "CONTROL"  & clinical_signs$value == TRUE,] %>%  .$UPCR1)$p.value
}
...
PUPD = 'Polyuria-Polydipsia'  Presenta evidencia per si sol. Els gats amb aquests símptomes tenen més probabilitts de tenir proteinuria.
...
clean_data %>%
  ...
  ungroup() %>% 
  filter(none) %>% 
  mutate(signs = n, no_signs = total - n) %>% 
  select(signs, no_signs) %>% 
  chisq.test()
```
...
clinical_signs %>% 
  ...
  filter(value) %>% 
  select(variable, protein) %>% 
  rename(`Clinical Signs` = variable) %>% 
  table() %>% 
  as_tibble() %>% 
  spread(protein, n) %>% kable()
```

Taula comptant el nombre de Borderlines, proteinurics i 'Normals' per cada signe clínic. El CONTROL vol dir 'cap signe'.

```{r}

cs <- clinical_signs$variable %>% unique()


cs_df <- clinical_signs %>% 
  mutate(protein = ifelse(UPCR1 < 0.2, "Normal", ifelse(UPCR1 < 0.4, "Borderline", "Proteinuria"))) %>% 
  filter(value) %>% 
  select(variable, protein) %>% 
  rename(`Clinical Signs` = variable) %>% 
  ...
  as_tibble() %>% 
  spread(protein, n) %>% 
  group_by(`Clinical Signs`) %>% 
  mutate(prot = Proteinuria, no_prot = sum(Borderline + Normal)) %>% 
  select(prot, no_prot, `Clinical Signs`) 


cs_pvalues <- c()
for(i in seq_along(cs[-3])){
  cs_pvalues[[i]] <- cs_df %>% 
    filter(`Clinical Signs` == cs[[i]] | `Clinical Signs` == 'CONTROL') %>% 
    ungroup() %>% 
    select(prot, no_prot) %>% 
    chisq.test() %>% 
    .$p.value
...
cs_df %>% 
  mutate(rate = prot / (prot + no_prot)) %>% 
  ggplot(aes(x = reorder(`Clinical Signs`, rate), y = rate)) +
  geom_col(fill = hp(3)[[2]]) +
  coord_flip() +
  geom_label(aes(label = rate %>% round(2))) +
  xlab('Clinical Signs') +
  ylab('Proportion of Proteinuria')



tibble(clinical_signs = cs[-3], pvalues = cs_pvalues) %>% 
  kable()

```


Aquí he repetit el mateix exercici, però enlloc de determinar si existeix diferencia en el valor d'UPC, he estudiat la diferencia entre les proporcions de proteinurics vs no-proteinurics. Al gràfic es poden veure la proporció de proteinurics en funció dels signes clínics que presenten. A la taula es veuen els p-values del resultat de fer el test Chi-Squared a cadascun d'ells vs el CONTROL (que és sense signes clínics).

Podem dir que tenen resultats significatius PUPD, Vomiting i Behaviour
...
clean_data %>%
  ...
  mutate(variable = case_when(
    variable == 'UPCR1' ~ 'UPC Ratio',
    variable == 'PCV1' ~ 'Total Packed Cell Volume',
    variable == 'Urea1' ~ 'Urea',
    variable == 'K1' ~ 'Potassium',
    variable == 'Crea_BQ1' ~ 'Creatinine',
    variable == 'Phos1' ~ 'Phosphorus',
    variable == 'Weight_1' ~ 'Body Weight',
    variable == 'Alb_1' ~ 'Albumin',
    variable == 'TP1' ~ 'Total Protein',
    variable == 'USG1' ~ 'Urine Specific Gravity',
    variable == 'Age_at_diagnosis_CKD' ~ 'Age',
    variable == 'BP1' ~ 'Blood Pressure',
    TRUE ~ variable
  )) %>% 
...
pvalues <- c()
for(i in seq_along(variables)){
	pvalues[[i]] <- wilcox.test( x = clean_data[clean_data$death, ][[variables[[i]]]],
									y = clean_data[!clean_data$death, ][[variables[[i]]]])$p.value
}
...
clean_data %>% 
  ...
  scale_x_discrete(labels = c('No CKD', 1, 2, 3, 4))
...
for(i in seq_along(treatments)){
  ...
  pvalues[[i]] <- wilcox.test(y,x)$p.value
}
...
hp_df <- clean_data %>% 
  select(BP1, UPCR1) %>% 
  mutate(hypertension = (BP1 >= 160)) %>% 
  na.omit()

hp_df %>% 
  mutate(hypertension = ifelse(hypertension, 'Hypertension', 'Non-Hypertension')) %>% 
  ggplot(aes(x = hypertension, y = UPCR1, fill = hypertension)) +
  geom_boxplot() +
  xlab('') +
  ylab('UPC Ratio') +
  scale_fill_hp_d(option = 'always', guide = FALSE, begin = 0.15, end = 0.45)

x <- hp_df %>% filter(hypertension) %>% .$UPCR1
y <- hp_df %>% filter(!hypertension) %>% .$UPCR1
